...
if [[ ! -e $DEST/.glance ]]; then
    ...
    source $DEST/.glance/bin/activate
fi
...
nkill glance
[[ -d $DEST/glance ]] && cp -rf $TOPDIR/source/glance/etc/* $DEST/glance/etc/
...
sed -i "/tarballs/d" $TOPDIR/source/glance/requirements.txt
sed -i "/tarballs/d" $DEST/glance/requirements.txt
...
cp -rf $TOPDIR/source/glance/etc/* /etc/glance/
...
sed -i "s,%KEYSTONE_GLANCE_SERVICE_PASSWORD%,$KEYSTONE_GLANCE_SERVICE_PASSWORD,g" $SERVICE_FILE
sed -i "s,%KEYSTONE_HOST%,$KEYSTONE_HOST,g" $SERVICE_FILE
...
sed -i "s,%GLANCE_DIR%,$DEST/glance,g" $SERVICE_FILE
VIR_DIR=$DEST/.glance
sed -i "s,%VIR_DIR%,$VIR_DIR,g" $SERVICE_FILE
