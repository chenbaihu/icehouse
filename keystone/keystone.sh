...
if [[ ! -e $DEST/.keystone ]]; then
    ...
    source $DEST/.keystone/bin/activate
fi
...
if [[ -d $DEST/keystone/etc ]]; then
    cp -rf $TOPDIR/source/keystone/etc/* $DEST/keystone/etc/
fi
...
sed -i "/MySQL/d" $KEYSTONE_DIR/requirements.txt
echo "MySQL-python" >> $KEYSTONE_DIR/requirements.txt
[[ ! -d $DEST ]] && mkdir -p $DEST
install_keystone
...
sed -i "s,# connection = .*,connection = $BASE_SQL_CONN/keystone?charset=utf8,g" $file
...
sed -i "s,%DEST%,$DEST,g" $temp_file

chmod +x /etc/init.d/keystone

rm -rf /tmp/tmp*
old_path=`pwd`
cd $DEST/.keystone/bin/
deactivate
cd $old_path
set +o xtrace
